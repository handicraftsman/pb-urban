    ...
    pb::command urban_cmd {
      ...
      .handler     = [] (pb::command& c, pb::event_command::ptr e) {
        ...
        if (q.definitions.size() > 0) {
          ...
          e->socket->stream() << pb::ircstream::reply(e, "&B" + term + ":&N " + def.def);
        } else {
          e->socket->stream() << pb::ircstream::reply(e, "No definitions available");
        }
      }
    };
    pb_plugin->register_command(urban_cmd);
  }
